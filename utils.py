...
from model.model import MIRROR, PATModel, CTCModel, RNNAttnSTR
...
def get_configs(is_training: bool):
    ...
    if is_training:
        ...
        # if os.path.exists(tfboard_path): # decision = input(f'{tfboard_path} has existed. Please choose the next
        # operation (d to delete | c for another creation | other to reuse)\n') decision = 'd' if decision == 'd':
        # shutil.rmtree(tfboard_path) elif decision == 'c': tfboard_path += '_' + time.asctime().replace(' ', '_')
...
def default_data(config_dict: dict, data_path, training=False, shuffle=False, distributed=False):
    dataset, dataloader = get_data(
        ...
        dict_path=config_dict['dict_path'],
    )
...
def get_model(config_dict, device, training=False, **kwargs):
    ...
    elif config_dict['model_type'] == 'mirror':
        net = MIRROR(
            num_classes=config_dict['num_classes'],
