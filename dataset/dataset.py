...
class ImageDataset(Dataset):
    ...
    def __init__(self,
        ...
        if char94:
            ...
            with open(dict_path, 'r', encoding='utf-8') as file:
                charlist = [line.strip() for line in file]
    ...
    def __getitem__(self, idx):
        ...
        if self.return_idx:
            return image, label, label_len, word, idx_new
        else:
            return image, label, label_len, word
...
def get_data(data_dir, img_h, img_w_max, max_len, batch_size, do_resize=False, data_aug=True, use_ctc=False,
             char94=False, workers=4, shuffle=False, is_train=False, distributed=False, dict_path=None):
    ...
    for data_dir_ in data_dir:
        dataset_list.append(
            ImageDataset(data_dir_, is_train, img_h, img_w_max, do_resize,
                         max_len, use_ctc, char94, convert_mode='RGB', data_aug=data_aug, dict_path=dict_path)
        )
    if len(data_dir) > 1:
    ...
    data_loader = DataLoader(dataset, batch_size=batch_size, num_workers=workers,
                             shuffle=shuffle, pin_memory=True, drop_last=is_train, collate_fn=AlignCollate(dataset),
