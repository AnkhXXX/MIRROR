...
from model.SpatialMLP import SpatialMLP


class ThreeDimensionalWeight(torch.nn.Module):
    def __init__(self, e_lambda=1e-4):
        super(ThreeDimensionalWeight, self).__init__()
        self.act = nn.Tanh()
...
class LocalSelfAttention(nn.Module):
    ...
    def _gen_t_index(self, real_len, device):
        idx = torch.stack([
            torch.arange(real_len, dtype=torch.long, device=device) + st for st in
            range(self.window_size)]).t()  # [T, w]
        return idx
    ...
    def forward(self, x, mask):
        ...
        alpha = q.unsqueeze(3).matmul(
            k.transpose(-1, -2) / self.head_dim * math.log(self.window_size))  # [b, T, h, 1, w]
        if self.mlm:
...
class SpatialLocalAttentionBlock(nn.Module):
    def __init__(self, feat_dim, nhead, window_size, add_pos_bias: bool, drop=0.0, proj_drop=0.0, init_values=1e-6,
                 mlm=False):
        super().__init__()
        self.norm1 = nn.LayerNorm(feat_dim)
        self.sa = LocalSelfAttention(feat_dim, nhead, window_size, add_pos_bias, drop, proj_drop, mlm=mlm)
        self.norm2 = nn.LayerNorm(feat_dim)
        self.smlp = SpatialMLP(size=feat_dim,
                              linear_units=2048,
                              kernel_size=31,
                              dropout_rate=drop,
    ...
    def forward(self, x, mask):
        ...
        x = x + self.gamma_2 * self.smlp(self.norm2(x))
        x = x * mask.unsqueeze(-1)
        return x


class SpatialLocalAttentionModule(nn.Module):
    def __init__(self, feat_dim, nhead, window_size, num_layers, drop_rate=0.0, proj_drop_rate=0.0, detach_grad=False,
                 mlm=False):
        super().__init__()
        self.attn_blocks = nn.ModuleList([
            SpatialLocalAttentionBlock(
        ...
        self.block = ThreeDimensionalWeight()

    def forward(self, x, mask):
        shut = x
        B, L, C = shut.size()
        # print(shut.size())
        shut = shut.view(B, C, 1, L)
        shut = self.block(shut)
        shut = shut.view(B, L, C)
...
if __name__ == "__main__":
    ...
    mask[:, :5] = 1.0
    blk = SpatialLocalAttentionModule(512, 8, 11, 2, 0.1, 0.1).cuda()
    y = blk(x, mask)
    print(y.size())
