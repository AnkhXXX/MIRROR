...
from model.MFAC import MFAC
from .feature_extractor import FocalNetBlock
from .la_utils import SpatialLocalAttentionModule
...
class FeatureMapBooster(nn.Module):
    ...
    def __init__(self, feat_dim, num_layers=1, focal_level=3, max_kh=1, layerscale_value=1e-6, drop_rate=0.0):
        ...
        self.block = MFAC(in_channels=feat_dim, out_channels=feat_dim, atrous_rates=[6, 12, 18])
    ...
    def forward(self, feat_map, feat_char, char_attn_map, vis_mask, h=1):
        ...
        re_feat_map = self.block(feat_map)
        feat_map = feat_map + re_feat_map
        return feat_map


class NextCharacterDecoder(nn.Module):
    ...
    def __init__(self, num_classes, feat_dim, max_ch=1000,
        ...
        self.iters = iters
        if iters > 0:
            self.cntx_module = SpatialLocalAttentionModule(
            ...
            self.merge_layer = FeatureMapBooster(feat_dim, num_layers=num_mg_layers)
...
if __name__ == "__main__":
    ...
    model = NextCharacterDecoder(37, 768, max_ch=25)
